...
import reducer, {
  ...
  useOverlaySnapshots,
} from "../../helpers/undo-redo";

import { DrawingActionKind } from "../../types/mapControl";

interface Props {
  drawingManager: google.maps.drawing.DrawingManager | null;
  circlesRef: React.RefObject<google.maps.Circle[]>;
  polygonRef: React.RefObject<google.maps.Polygon[]>;
}
...
export const UndoRedoControl = ({
  ...
  const handleUndo = () => {
    ...
    const lastCircle = circlesRef.current.pop();
    if (lastCircle) {
      lastCircle.setMap(null); // Remove from map
    }
    const lastPolygon = polygonRef.current.pop();
    if (lastPolygon) {
  ...
  const handleRedo = () => {
    ...
    const lastCircle = state.now[state.now.length - 1];
    if (lastCircle) {
      circlesRef.current.push(lastCircle);
      lastCircle.setMap(map); // Re-add to map
    }

    const lastPolygon = state.now[state.now.length - 1];
    if (lastPolygon) {
      polygonRef.current.push(lastPolygon);
      lastPolygon.setMap(map); // Re-add to map
    }
  };

  return (
    <div className="drawing-history">
      <button onClick={handleUndo} disabled={!state.past.length}>
        <svg
      ...
      <button onClick={handleRedo} disabled={!state.future.length}>
        <svg
          xmlns="http://www.w3.org/2000/svg"
          height="24"
          viewBox="0 -960 960 960"
